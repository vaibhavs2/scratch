...
export default function ConfigureScreen(props: Props) {
  ...
  const flatlistLayout = useRef<LayoutRectangle>({
    height: 0,
    width: 0,
    x: 0,
    y: 0,
  ...
  const panResponder = React.useRef(
    PanResponder.create({
      ...
      onPanResponderGrant: (evt, gestureState) => {
        ...
        Animated.event([{y: pressedPointer.y, x: pressedPointer.x}], {
          useNativeDriver: false,
        })({y: gestureState.x0 - flatlistLayout.current.y, x: gestureState.x0});
        let index = Math.floor(
          (scrollOffset.current + gestureState.y0 - flatlistLayout.current.y) /
            60,
        );
        if (index < 0) index = 0;
      ...
      onPanResponderMove: (evt, gestureState) => {
        Animated.event([{y: pressedPointer.y, x: pressedPointer.x}], {
          useNativeDriver: false,
        })({y: gestureState.moveY, x: gestureState.moveX});
  ...
  return (
    <SafeAreaView style={styles.screenContainer}>
      <TouchableOpacity
        style={styles.configHeaderContainer}
      ...
      {isItemDraggingIndex != -1 && (
        <Animated.View
          style={{
            ...
            transform: [
              {translateX: pressedPointer.x},
              {translateY: pressedPointer.y},
            ],
          }}>
            ...
            <View
              ...
              {!item.isHeader && (
                <View
                  ...
                  style={styles.dragabblePoint}>
                  <Text style={styles.point}>@</Text>
                </View>
              )}
              <Text
                style={item.isHeader ? styles.headerTitle : styles.itemTitle}>
                {item.name}
              </Text>
            </View>
          )}
