...
export function AnimatebleComponent(props: Props) {
  ...
  const panResponder = useRef(
    PanResponder.create({
      ...
      onPanResponderMove: (evt, gestureState) => {
        Animated.event([{y: linearTransitionXY.y, x: linearTransitionXY.x}], {
          useNativeDriver: false,
        })({y: gestureState.moveY, x: gestureState.moveX - 60});
  ...
  const xTransition = (nextEventIndex: number) => {
    Animated.timing(linearTransitionXY, {
      toValue: {
        x: linearTransitionXY.x._value + 50,
        y: linearTransitionXY.y._value,
      },
      duration: 800,
      easing: Easing.linear,
      useNativeDriver: false,
  ...
  const yTransition = (nextEventIndex: number) => {
    Animated.timing(linearTransitionXY, {
      toValue: {
        x: linearTransitionXY.x._value,
        y: linearTransitionXY.y._value + 50,
      },
      duration: 800,
      easing: Easing.linear,
      useNativeDriver: false,
  ...
  return (
    <Animated.View
      style={[
        linearTransitionXY.getLayout(),
        {
          transform: [{rotate: rotateView}, {scale: scale}],
          width: 60,
          height: 60,
        },
      ]}
